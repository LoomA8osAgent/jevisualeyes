...
| **I5.5** | **THE SHARED SHADING STACK — one composed look set that shades every model.** Not per-formula presets: the accordion the card labels SHADING (group `surface`) and exactly the children the code nests under it (`_groupParent: 'surface'` — color · palette · texture · material · lighting → light 1/2/3 · structure · edge) are emitted by the SHARED template, identical on every record, so ONE composed set is shipped through the app's existing factory-bank surface (`a8_sdf_factory_banks`) and every model inherits it at compose time with no rebake and no per-record bank key. Blend modes, bindings, per-slider presets and the marching accordion are OUT, each by a mechanical exclusion. **ACCEPTANCE CRITERION (operator, 2026-09-20 13:57, verbatim): "this is EXACTLY to expose as many DIFFERENT looks as possible so I do not have to discover them all."** Every child bank's 11 slots must map bijectively onto the 11 surface looks (no repeats), every menu row's every option must appear somewhere in its bank's 11 slots wherever 11 slots make that mathematically possible, ranged knobs must spread farthest-point rather than merely clear a diversity floor, and the light rig must vary how many lights are on (not mostly-all-on) with lit lights differing in kind from each other within one look. | The consuming app's user-path macro `sdf.shading-factory-recall`: Library → a resolver-picked model → the SHADING accordion's bank is visible on load with the composed slots → a real click on slot 1 → every param the slot carries is on the engine and all ten child banks are lit on the slot the look recorded, one recompile, the fold flushed within 60 ms. Local provider (Laya), light tier. Plus the render gate: every delivered slot renders non-black on the subject (`core/render-gate.ts` over the app's headless renderer). | **DELETED** (operator 2026-09-20 16:43: *"delete the full factory preset stack, it's useless"*). What shipped was random knob values inside ranges with a decision model choosing among random draws, passed by a render floor that called luma 10/255 visible; it never composed the stack's MODES (which layers are in, the light rig, palette mapping, material model, structure and edge modes, with knobs at the values those modes need) and the eye that was built to judge frames was never in the loop. The composer files (`shading*.ts`, `render-gate.ts`, `compose/deliver/relabel-shading.mjs`, their tests) are removed; the app-side mechanism (tracked factory home, mint-on-merge, childSlots, visible banks, batched recall, headless renderer) stands empty. A replacement is written as spec from the stack's own descriptors and gates, with every slot judged on a rendered frame, before any value is chosen — and only when the operator says |
...
| **I7** | **Bake the corpus** — defined by the operator (2026-09-20): *the formulas bring their own geometry variables to the transform, world and shading stacks — and the camera; they already have marching rules.* So the four shared stacks each get a factory look set first (I5.5 is shading; transform, world and camera follow its shape), inherited by every record, and the bake composes only each record's own geometry knobs against them. 495 records: the run, its concurrency, its resume, its report. | The run completes and the report **names every skipped record and why**; a spot sample of composed slots loads through I5's macro. | planned, gated on the three remaining shared stacks + the per-slider knob presets |
...
**WHAT LANDED, composer side — the THIRD-draft mechanism (two coordinator audits + four operator
rulings, all 2026-09-20, landed in one pass).** `core/rosters.ts` gains `shading` as a 5th
`shared.*` shape (a group tree alongside the flat `RosterInput` list) plus `constraints` — the
compatibility-derivation lane's own object (`{method, axes, baseDrivers, entries, glslNotes,
combinations}`; `entries` is the per-combination verdict list this repo filters against — READ,
never transcribed, and never waited on synchronously inside a roster read). `core/shading.ts` is
the bank/knob-filtering half (`bankKnobs` applies the two mechanical exclusions). Three mechanisms
now do the actual work, replacing the second draft's Laya-picks-content-then-code-checks-a-floor
design entirely:

1. **Spread, inside each bank's own 11 slots — CODE-ONLY, zero network.** `stratifiedBankSlots`
   (`core/shading-samplers.ts`) over-samples ~500 candidate draws, then greedily covers every
   `menuRows` (bool/enum `_menuOnly` knob) option first — tie-broken by farthest-point distance
   from what's already picked — and fills any remainder by pure farthest-point distance
   (`bankDistance`, kept from the second draft). A bank whose own state space is smaller than 11
   (`sub:lighting`'s 3 bools ⇒ 8 states) takes every distinct state and pads with REPORTED
   duplicates (`duplicates: [{slot, duplicateOfSlot}]`) — never a forced-accept floor pretending 8
   states are 11 different ones. `assertFullCoverage` in the compose script re-runs this check and
   THROWS (refuses to deliver) if any coverable row (option count ≤ 11) is missing an option
   anywhere in the delivered set — the same check is a pure vitest test
   (`tests/shading-coverage.test.ts`, no network) so a broken run REDS before it ever calls Laya.
2. **Partition, across the 11 surface looks — every child slot used in EXACTLY one look.** The
   second draft's `vectorSignature` retry-on-collision design still let a bank repeat a slot across
   looks; the operator's ruling (*"the whole point of this exercise is to NOT have duplicate
   looks"*) replaced it structurally: `newPartitionState`/`availableSlots`/`forcedSlot`/`commitPick`
   shrink a bank's menu by exactly the slots already used in looks 1..k-1, so look 11 always sees a
   singleton menu and it is FORCED, never asked (`docs/COMPOSER.md`'s own "a single-option menu is
   not a decision" rule, applied one level up). Proven without a network call in
   `tests/shading-partition.test.ts` (three different stub choosers, all yield the same {1..11}
   invariant — the property belongs to the reduction, not to what a model happens to pick).
3. **Coherence.** `spreadShadingCoordinates` (unchanged from the second draft — 11 distinct
   axis-word combos, seed-shuffled off `axes.ts AXES`) still gives each look a genuine target. The
   ASK ORDER is now constraining-first (`sub:lighting` → `sub:material` → the three lights →
   everything else, operator: *"order the child questions so the constraining banks … are asked
   before the constrained ones"*). Two more rules ride the same per-look loop: an `extraExclude`
   closure keeps a look's lit lights from sharing a `light{n}Type` with an already-chosen lit light
   in the SAME look (falls back to the unfiltered menu rather than deadlock when the exclusion would
   empty it — visible in the delivered set as look 11's forced type-8/type-8 collision on the
   partition's OWN last slot, reported honestly below); and `violatesConstraint`/`isDarkened`
   exclude any child slot that would complete an `incompatible`/`inert` combination the constraints
   lane found, or skip Laya entirely for a group the constraints lane says has NO LIVE ROW at all
   under what's already chosen (`darkGroups` — every slot of a fully-dark group is equally inert,
   so there is nothing to prefer among them). `tests/shading-constraints.test.ts` proves the FILTER
   mechanism against a clearly-marked PLANTED fixture (the real field landed mid-session, so the
   live `constraintRemovals` log in the delivered artifact is the live proof, not a fabricated one).
   Surface's OWN ~12 knobs (the layer toggles) are now ALSO `stratifiedBankSlots` output, one set
   per look directly (no partition — they are never reused via `childSlots`), which is what makes
   surface's own 7 bool menu rows satisfy the SAME hard coverage gate as every child bank.

`scripts/compose-shading.mjs` runs Phase 1 (stratify all 10 children + surface's own knobs, zero
network) → the hard coverage gate → Phase 2 (the 11-look constraining-first partition loop above),
against the LOCAL provider (`von serve` on loopback, model responded `von-1.0.0`) — the live run
against the real constraints field: 91 requests, 91 receipts, 15 constraint entries read, 16
offers removed by them, 19 forced/skipped picks (partition singletons + constraint/type
exclusions), zero fixture, zero hand-tuning. `scripts/deliver-shading.mjs` is unchanged in
mechanism (rewrites only the `var BANKS = { … };` literal, verified by `require()` read-back);
this run additionally verified all 121 labels valid and, per child bank, the used-slot set across
the 11 surface looks equals exactly `{1..11}`. `tests/fixtures/rosters.json`'s `shared.shading`
stub is unchanged; 111/111 green (84 pre-existing + 6 partition + 15 coverage + 6 constraints),
`tsc --noEmit` clean.
...
the file this run wrote. Look 11's `sub:light2`/`sub:light3` picks share `light{n}Type` 8 — the
partition's OWN last slot for each bank is FORCED (menu of one) regardless of the type-uniqueness
`extraExclude`, so a same-type collision on the very last look is an expected, reported endgame
effect of guaranteeing full partition coverage, not a bug in the exclusion logic.

...
**Status: increments I1 · I2 · I3 · I4 · I5 have landed** (`docs/PLAN.md` §1) — a composed unit is written into the consuming application's own preset bank and recalled there on the user path; I6 is next. The engine lineage and its MIT notice are in `LICENSE`; nothing else of the origin remains in this tree. What this repository IS: the composer loop (persist-before-call → validate → one durable commit → receipt), the fixture-provider posture, strict response validation, and the resumable single-runner job engine — pointed at preset composition for the A8os shape corpus. A8os is a desktop application (Tauri); this repository is the offline author-time tool beside it, not a shipped product — it has no UI and no server to visit, and it renders nothing itself: every preset it composes is delivered into the application, which renders it.

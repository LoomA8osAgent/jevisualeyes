...
import {PPQ,STRAIGHT_SPANS,canonicalJSON,hashJSON,barTicks,warpTick,performedNote,pitchWindow,legalSpans,buildCandidates,criteriaFor,validateChoiceResponse,nextRandom,selectChoice,applyEvent,nextLane,canCommit,assertPreserved,validateProject,encodeSMF,parseSMF} from '../reference/core.mjs';
const load=n=>JSON.parse(readFileSync(new URL('../examples/'+n,import.meta.url),'utf8'));
const base=load('hand-authored-project.json'),request=load('jev-request.json'),response=load('jev-response.synthetic.json');
const copy=x=>structuredClone(x);
const note=(id,start=0,len=480,pitch=69,locked=false)=>({id,startTick:start,durationTicks:len,midi:pitch,velocity:84,locked,source:{kind:'fixture',decisionIds:[],parentNoteIds:[]}});
...
test('provider: nonmaximum reported choice is rejected',()=>{const r=copy(response);r.answers.next_event.choice='R_120';assert.throws(()=>validateChoiceResponse(r,request));});
test('sampling: zero temperature tie-break is stable',()=>{const a={choice:'b',probabilities:{b:.5,a:.5}};assert.equal(selectChoice(a,{mode:'sample',temperature:0,seed:2}).selected,'a');});
test('sampling: model mode uses provider choice without advancing PRNG',()=>{assert.deepEqual(selectChoice({choice:'b',probabilities:{a:.5,b:.5}},{mode:'model',seed:7}),{selected:'b',seed:7});});
test('sampling: same saved distribution and seed replay exactly',()=>{const a={choice:'a',probabilities:{a:.7,b:.3,c:0}};let s1=77,s2=77;for(let i=0;i<100;i++){const x=selectChoice(a,{mode:'sample',seed:s1});const y=selectChoice(a,{mode:'sample',seed:s2});assert.deepEqual(x,y);assert.notEqual(x.selected,'c');s1=x.seed;s2=y.seed;}});
...
test('canonical JSON: unsafe keys, cycles, nonfinite values rejected',()=>{assert.throws(()=>canonicalJSON(JSON.parse('{"__proto__":1}')));assert.throws(()=>canonicalJSON({x:Infinity}));const a={};a.a=a;assert.throws(()=>canonicalJSON(a));});
test('events: note appends and advances without mutating input',()=>{const ns=[];const c=buildCandidates({pitches:[69],spans:[480],remaining:480})[0];const r=applyEvent(ns,0,c,'d1');assert.equal(ns.length,0);assert.equal(r.notes[0].midi,69);assert.equal(r.cursorTick,480);});
test('events: rest advances timeline without creating a MIDI note',()=>{const r=applyEvent([],0,{kind:'rest',stepTicks:240},'d1');assert.equal(r.notes.length,0);assert.equal(r.cursorTick,240);});
test('events: hold extends sound without retrigger and adds provenance',()=>{const r=applyEvent([note('n')],480,{kind:'hold',stepTicks:240,targetNoteIds:['n']},'d2');assert.equal(r.notes.length,1);assert.equal(r.notes[0].durationTicks,720);assert.deepEqual(r.notes[0].source.decisionIds,['d2']);});
test('events: hold cannot target a locked or no-longer-sustained note',()=>{for(const n of [note('n',0,480,69,true),note('n',0,240)])assert.throws(()=>applyEvent([n],480,{kind:'hold',stepTicks:240,targetNoteIds:['n']},'d2'));});
...
test('scheduler: earliest cursor then documented role order',()=>{const roles={a:'lead',b:'bass',h:'harmony',d:'drums'};assert.equal(nextLane({a:0,b:0,h:0,d:0},roles),'d');assert.equal(nextLane({a:0,b:480,h:480,d:480},roles),'a');});
...
test('MIDI: oracle bytes equal the packaged hand-authored file',()=>{const b=readFileSync(new URL('../examples/hand-authored-example.mid',import.meta.url));assert.deepEqual(encodeSMF(base),b);});
...
test('events: duplicate hold receipt cannot extend a sound twice',()=>{const c={kind:'hold',stepTicks:240,targetNoteIds:['n']};const a=applyEvent([note('n')],480,c,'d2');assert.throws(()=>applyEvent(a.notes,720,c,'d2'));});
